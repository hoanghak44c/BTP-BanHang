...
{
    public partial class frmBH_LapDonHangBanOnline : DevExpress.XtraEditors.XtraForm
    {
        #region "Khai báo biến"

        private LoaiGiaoDichBanHang LoaiGiaoDich = LoaiGiaoDichBanHang.LAP_DONHANG_ONLINE;
        ...
        private int TrangThaiDonHang = (int)OrderStatus.TAO_DON_HANG_ONLINE;
        ...
        public frmBH_LapDonHangBanOnline()
        {
            InitializeComponent();
        ...
        public frmBH_LapDonHangBanOnline(int loaiChungTu)
        {
            InitializeComponent();
            Common.LoadStyle(this);
        ...
        public frmBH_LapDonHangBanOnline(ChungTuBanHangInfor chungTuBanHangInfor)
        {
            InitializeComponent();
            Common.LoadStyle(this);
        ...
        private bool IsEnableCommand(LoaiGiaoDichBanHang type)
        {
            if (type == LoaiGiaoDichBanHang.LAP_DONHANG_ONLINE)
                return false;
        ...
        private bool ValidChungTuCommonInfor()
        {
            ...
            if (!BanHangDataProvider.Instance.CheckValid_TrangThai_ChungTu(Business.ChungTu.IdChungTu, "1,3"))//trang thai dang xuat hoac da xuat
            {
            ...
            {
                MessageBox.Show("Phải chọn khách hàng", Declare.titleNotice, MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        private bool ValidHinhThucThanhToan(ChungTuBanHangChiTietInfor ct)
        {
            ...
            {
                if (ctu.IdHinhThucThanhToan == 0 || String.IsNullOrEmpty(ctu.HinhThucThanhToan))
                {
                    MessageBox.Show(String.Format("Phiếu thu {0} chưa chọn hình thức thanh toán", ctu.SoPhieu));
                    return false;
                }
            }
        ...
        private bool ValidSoLuongDonHang(ChungTuBanHangChiTietInfor ct)
        {
            ...
            //            shop.SoLuongDonHang <= BanHangDataProvider.Instance.SoChungTuDaMua((shop.IdTrungTam != 0 ? shop.IdTrungTam : -1), (shop.IdKho != 0 ? shop.IdKho : -1), ct.MaSanPham, 6, ct.ChinhSach.NgayHieuLuc, ct.ChinhSach.HanHieuLuc, "", ""))
            //        {
            //            MessageBox.Show(
            //                String.Format(
            //                    "Số lượng đơn hàng đã vượt giới hạn cho phép [{0} đơn hàng của kho {1} - Trung tâm: {2}]",
            //                    shop.SoLuongDonHang, shop.TenKho, shop.TenTrungTam), Declare.titleNotice,
...
//        }
        ...
        {
            this.Text = CommonFuns.Instance.GetOrderInfor(Business.ChungTu, Updating, LoaiGiaoDichBanHang.LAP_DONHANG_ONLINE);
        ...
        private void InputDataFromCode(string code)
        {
            try
            {
                ...
                //    Business.ChungTu.SoChungTu.Contains("XNOL") &&
                ...
                else
                {
                    ...
                    ((BindingList<ChungTuBanHangChiTietInfor>)gSanPhamBan.DataSource).ResetBindings();
                    //add khuyen mai
                    CommonFuns.Instance.LoadChinhSachKM(ct, IdTTamSelected, IdKhoSelected, _IdLoaiKhachHang,
                                                        (int) LoaiKhachHang.KHACH_ONLINE,
                                                        gSanPhamBan, Business.ListChiTietChungTu, true);
                }
        ...
        private void ThemMoiDonHang(ChungTuBanHangInfor old)
        {
            ...
            LoaiChungTu = (int) TransactionType.DON_HANG_ONLINE;
            ...
            txtSoOrderKH.Prefix = Declare.Prefix.TaoDonHangOnline;
            txtSoOrderKH.ReSet();//.Text = CommonProvider.Instance.GetSoPhieu(Declare.Prefix.TaoDonHangOnline);
        ...
        {
            if (Business.ChungTu.IdChungTu == 0 || Business.ChungTu.IdChungTu > 0 && (TrangThaiDonHang == (int)OrderStatus.REJECT_DON_HANG_ONLINE ||
                    TrangThaiDonHang == (int)OrderStatus.TAO_DON_HANG_ONLINE && (Draft == 1 || nguoiDung.SupperUser == 1)))
            {
                return true;
            }
            return false;
        ...
        private void setEDItems()
        {
            ...
            txtSoChungTuGoc.ReadOnly = true;// !Updating;
        ...
        private bool UpdateChungTu(bool isIn)
        {
            ...
            //double tienNoOld = -Business.ChungTu.TienThanhToanThuc;ư
            ...
            try
            {
                if (ValidChungTuInfor())
                {
                    ...
                    if (!isIn)
                    {
                        if (IsAdded)
                        {
                            MessageBox.Show("Lập đơn hàng bán Online thành công!");
                        }
                        else
                        {
                            MessageBox.Show("Cập nhật đơn hàng bán Online thành công!");
                        }
                    }
                    return true;
                }
        ...
        {
            if (Draft != 0 && (TrangThaiDonHang == (int)OrderStatus.TAO_DON_HANG_ONLINE || TrangThaiDonHang == (int)OrderStatus.REJECT_DON_HANG_ONLINE))
            {
                return true;
            }
            return false;
            ...
            try
            {
                if (MessageBox.Show("Bạn có chắc chắn muốn hủy đơn hàng này không (Yes/No)?", "Cảnh báo", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    ...
                    if (Business.ChungTu.IdChungTu > 0)
                    {
                        ...
                        //BanHangDataProvider.Instance.UpdateCongNoKhachHang(Business.ChungTu.IdKhachLe, Business.ChungTu.IdDoiTuong);
                    }
                    ConnectionUtil.Instance.CommitTransaction();
        ...
        {
            //hah: check đơn hàng đã xác nhận thì không sửa
            //if (!String.IsNullOrEmpty(Business.ChungTu.SoChungTu) && Business.ChungTu.SoChungTu.Contains("XNOL") &&
            //    !BanHangDataProvider.Instance.CheckValid_TrangThai_Draft(Business.ChungTu.IdChungTu))
            //{
        ...
        private void bteKhoXuat_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (UtilFunctions.bteKhoXuatNhapLai_KeyDown(sender, e, bteKhoXuat, -1, nguoiDung.IdNhanVien))
            {
                if (gvSanPhamBan.RowCount > 0)
                {
                    if (MessageBox.Show("Dữ liệu đang nhập, nếu chuyển thông tin thì dữ liệu đã nhập sẽ bị xóa\nBạn có đồng ý đổi thông tin hay không?", "Thông báo", MessageBoxButtons.YesNo) == DialogResult.Yes)
        ...
        private void bteKhoXuat_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            ...
            if (UtilFunctions.bteKhoXuatNhapLai_ButtonClick(sender, e, bteKhoXuat, Declare.IdTrungTam, nguoiDung.IdNhanVien))
            {
                if (gvSanPhamBan.RowCount > 0)
                {
                    if (MessageBox.Show("Dữ liệu đang nhập, nếu chuyển thông tin thì dữ liệu đã nhập sẽ bị xóa\nBạn có đồng ý đổi thông tin hay không?", "Thông báo", MessageBoxButtons.YesNo) == DialogResult.Yes)
        ...
        private void frmBH_LapDonHangBanOnline_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (Business.ChungTu.IdChungTu > 0 && Business.ChungTu.Draft == 1 && MessageBox.Show("Đơn hàng mới lưu nháp, chưa được xác nhận!!\n\nCác bộ phận khác sẽ không nhìn thấy được!!!\n\nBạn có đồng ý thoát chức năng này không? (Yes/No)?", "Cảnh báo", MessageBoxButtons.YesNo) == DialogResult.No)
            {
                CommonProvider.Instance.Lock_Unlock_ChungTu(Business.ChungTu.IdChungTu, 0);//mo chung tu                    
