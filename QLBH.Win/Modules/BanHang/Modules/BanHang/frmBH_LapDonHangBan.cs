...
{
    public partial class frmBH_LapDonHangBan : DevExpress.XtraEditors.XtraForm
    {
        ...
        public frmBH_LapDonHangBan(int loaiChungTu)
        {
            InitializeComponent();
            Common.LoadStyle(this);
        ...
        public frmBH_LapDonHangBan(ChungTuBanHangInfor chungTuBanHangInfor)
        {
            InitializeComponent();
            Common.LoadStyle(this);
        ...
        private void ResetChungTuInfor(ChungTuBanHangInfor oldCT)
        {
            try
            {
                ...
                bteKhachLe.Tag = (oldCT == null ? null : DmDoiTuongProvider.GetDmDoiTuongInfoFromOid(oldCT.IdKhachLe));
        ...
        private bool ValidChungTuInfor()
        {
            ...
            {
                if ((ct.IdSanPhamBan == 0/* || ct.IdSanPhamBan > 0 && ct.LoaiHangKem != 0*/) && (ct.ChietKhau != 1) && (ct.DonGiaCoVat == 0 && ct.GiaTheoBangGia == 0 || ct.ThanhTien == 0/*tongTienThanhToan == 0*/) && nguoiDung.SupperUser != 1)
                {
            ...
            try
            {
                if (ValidChungTuInfor())
                {
                    ...
                    if (!isIn)
                    {
                        if (IsAdded)
                        {
                            MessageBox.Show("Lập đơn hàng bán thành công!");
                        }
                        else
                        {
        ...
        private void UpdateChungTuThanhToan()
        {
            ...
            {
                if ((ctu.CSachChietKhau != null || ctu.TyLeCSachChietKhau != 0) && Draft == 0)//khong tinh den ma hang chinh sach chiet khau
                    continue;
        ...
        private void UpdateSummariesInfors()
        {
            try
            {
                ...
                foreach (ChungTuBanHangChiTietInfor ctu in (BindingList<ChungTuBanHangChiTietInfor>)gSanPhamBan.DataSource)
                {
                    if (ctu.CSachChietKhau != null || ctu.TyLeCSachChietKhau != 0)//khong tinh den ma hang chinh sach chiet khau
                        continue;
            ...
            try
            {
                ...
                foreach (ChungTuBanHangChiTietInfor ct in Business.ListChiTietChungTu)
                {
                    if (ct.CSachChietKhau != null)
                    {
                        ...
                        tyLeCK += ct.CSachChietKhau.TyLeChietKhau;
